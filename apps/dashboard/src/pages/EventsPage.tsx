...
export function EventsPage() {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
      <Topbar
        ...
        actions={
          <select
            ...
            className="rounded-xl border border-zinc-800 bg-zinc-900 px-3 py-2 text-sm font-medium text-zinc-200 transition focus:border-brand-violet focus:outline-none focus:ring-4 focus:ring-brand-violet/15"
          >
            {EVENT_TYPES.map(t => (
      ...
      <div className="grid min-h-0 flex-1 grid-cols-2 gap-0 overflow-hidden bg-ambient">
        {/* Left: live feed */}
        <div className="flex flex-col overflow-hidden border-r border-zinc-800 bg-zinc-900">
          <div className="flex items-center gap-2 border-b border-zinc-800 px-5 py-3.5">
            <span className="h-1.5 w-1.5 rounded-full bg-brand-violet shadow-[0_0_8px_#a855f7]" />
            <span className="text-[13px] font-bold text-zinc-50">Live Feed</span>
            {paused && (
              <button
                onClick={() => { setPaused(false); feedRef.current?.scrollTo({ top: 0, behavior: 'smooth' }); }}
                className="ml-auto flex items-center gap-1.5 rounded-full border border-zinc-800 px-2.5 py-0.5 text-[11px] font-medium text-zinc-400 transition hover:bg-zinc-800/40"
              >
          ...
          <div ref={feedRef} className="flex-1 overflow-y-auto divide-y divide-zinc-800/50">
              ...
              <div key={e.id} className="flex items-start gap-3 px-5 py-3.5">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-[13px] font-semibold text-zinc-50">
                    ...
                    {e.isNew && (
                      <span className="ml-1.5 rounded bg-brand-violet/15 px-1 py-0.5 text-[9px] font-bold uppercase tracking-wide text-brand-violet">new</span>
                    )}
                  </p>
            ...
            {liveEvents.length === 0 && (
              <div className="flex flex-col items-center justify-center gap-2 py-20 text-sm text-zinc-500">
                <span className="h-2 w-2 animate-pulse rounded-full bg-brand-violet/50" />
                Waiting for events…
              </div>
            )}
          </div>
        ...
        <div className="flex flex-col overflow-hidden bg-zinc-900">
          <div className="flex items-center border-b border-zinc-800 px-5 py-3.5">
            <span className="text-[13px] font-bold text-zinc-50">History</span>
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-zinc-800/50">
            {historyData?.data.map(e => (
              <div key={e.id} className="flex items-start gap-3 px-5 py-3.5">
                <EventDot type={e.eventType} />
                <div className="min-w-0 flex-1">
                  <p className="text-[13px] font-semibold text-zinc-50">{e.employeeNo ?? 'Unknown'}</p>
            ...
            {historyData?.next_cursor && (
              <button
                ...
                disabled={isFetching}
                className="w-full py-3 text-xs font-medium text-brand-violet transition hover:text-brand-fuchsia disabled:opacity-50"
              >
                {isFetching ? 'Loading…' : 'Load more'}
              </button>
            )}
            {!isFetching && historyData?.data.length === 0 && (
              <div className="py-20 text-center text-sm text-zinc-500">No events found.</div>
            )}
          </div>
        </div>
      </div>
