...
/** A raw attendance row written to BoldGym's `ScanLog` collection. */
export interface AttendanceRecord {
  memberId: string;
  gate: 'gym' | 'beach';
  result: 'granted' | 'denied' | 'expired' | 'duplicate' | 'invalid';
  reason: string | null;
  scannedAt: Date;
...
/** Normalized Keystone door event, vendor-agnostic. */
export interface DoorEvent {
  memberId: string;
  deviceId: string | null;
  eventTime: Date;
  /** Physical direction if the device reports it; recorded in `reason`. */
  direction?: 'entry' | 'exit';
}

/**
 * Map a resolved door event to a BoldGym `ScanLog` row. Keystone only forwards
 * events for members it recognizes (already linked + on-device), so these are
 * `granted`; the entry/exit direction rides in `reason` (ScanLog has no
 * direction field), leaving analytics to BoldGym.
 */
export function doorEventToAttendance(event: DoorEvent): AttendanceRecord {
  return {
    memberId: event.memberId,
    gate: 'gym',
    result: 'granted',
    reason: event.direction ?? null,
    scannedAt: event.eventTime,
